...
def render_backtest_tab(
    *,
    ...
    with col_start:
        start_date = st.text_input("Start (YYYY-MM-DD)", "", key="bt_start", placeholder="Optional")
    
    with col_end:
        end_date = st.text_input("End (YYYY-MM-DD)", "", key="bt_end", placeholder="Optional")
    ...
    with col_load:
        if st.button("📥 Load from Production Config", use_container_width=True):
    ...
    with col_gen:
        ...
        if st.button("🔮 Generate Predictions CSV", use_container_width=True):
            try:
    ...
    with col_save:
        if st.button("💾 Deploy to Production Config", type="primary", use_container_width=True):
            # Validation
            if rr_val <= 0.0:
    ...
    # ==========================================================================
    # SECTION 3: RUN BACKTEST / OPTIMIZE
    # ==========================================================================
    st.markdown("### 🚀 Execute")
    ...
    # Use component for history table
    components.render_history_table(
        st,
        pd,
        history=history_for_view,
        title=f"Backtest History ({freq})",
        columns=["timestamp", "trade_side", "sharpe_ratio", "total_return", "max_drawdown", "n_trades"],
        on_row_select=None
    )

    with st.expander("📜 All History (All Frequencies)", expanded=False):
        if history_all:
    ...
    if active_job_id:
        ...
        with col_cancel:
            if st.button("🔄 Refresh", key="opt_job_refresh", use_container_width=True):
                st.rerun()
    ...
    if results_df is not None and not results_df.empty:
        ...
        with col_load_best:
            if st.button("⭐ Load Best (Sharpe)", use_container_width=True, type="primary"):
        ...
        with col_export:
            ...
            if st.button("📤 Export to Walk-Forward", use_container_width=True):
    ...
    components.render_history_table(
        ...
        title=f"Optimization History ({freq})",
