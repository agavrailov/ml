...
def test_load_json_history_missing_file_returns_empty(tmp_path: Path, monkeypatch) -> None:
    """load_json_history should return [] when the file does not exist."""

    # Point UI_STATE_DIR at a temporary directory with no history files.
    monkeypatch.setattr(ui_state, "UI_STATE_DIR", tmp_path)

    history = ui_state.load_json_history("backtests_history.json")
    assert history == []


def test_save_and_load_json_history_round_trip(tmp_path: Path, monkeypatch) -> None:
    """save_json_history writes JSON that load_json_history can read back."""
    ...
    ui_state.save_json_history("backtests_history.json", rows)

    path = tmp_path / "backtests_history.json"
    assert path.exists()

    loaded = ui_state.load_json_history("backtests_history.json")
    assert loaded == rows


def test_save_json_history_truncates_to_max_history_rows(tmp_path: Path, monkeypatch) -> None:
    """save_json_history should keep only the most recent MAX_HISTORY_ROWS entries."""

    monkeypatch.setattr(ui_state, "UI_STATE_DIR", tmp_path)

    # Force a small cap so the test is cheap.
    monkeypatch.setattr(ui_state, "MAX_HISTORY_ROWS", 3)
...
def test_filter_backtest_history_by_frequency_sorts_desc() -> None:
    ...
    filtered = formatting.filter_backtest_history(rows, frequency="1min")
...
def test_filter_optimization_history_by_frequency_sorts_desc() -> None:
    ...
    filtered = formatting.filter_optimization_history(rows, frequency="1min")
...
def test_run_backtest_raises_when_predictions_csv_missing(monkeypatch) -> None:
    ...
    with pytest.raises(FileNotFoundError):
        run_backtest_for_ui(
            frequency="1min",
            prediction_mode="csv",
            predictions_csv="C:/tmp/missing_predictions.csv",
            start_date=None,
            end_date=None,
