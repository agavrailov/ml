"""Walk-forward evaluation of the top-N strategy parameter sets for MSFT.
...
Usage (from repo root):
    python -m scripts.walkforward_param_sweep_msft
    python -m scripts.walkforward_param_sweep_msft --top 20 --test-span-months 3
"""
...
GRID_RESULTS_CSV = os.path.join("backtests", "msft_grid_search_top20.csv")
...
def run_sweep(
    ...
    if not os.path.exists(GRID_RESULTS_CSV):
        raise FileNotFoundError(
            f"Grid search results not found at {GRID_RESULTS_CSV}.\n"
            "Run the grid search first:\n"
            "  python -m scripts.grid_search_strategy_msft "
            f"--no-save --save-results {GRID_RESULTS_CSV}"
        )

    grid_df = pd.read_csv(GRID_RESULTS_CSV)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Walk-forward sweep of top MSFT strategy param sets.")
