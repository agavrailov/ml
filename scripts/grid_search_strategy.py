"""Strategy parameter grid search for any symbol (60min, bidirectional).

Sweeps k_sigma_long/short and k_atr_long/short over a coarse grid, ranks
results by Sharpe ratio, and optionally saves the best combo to
``configs/symbols/{SYMBOL}/active.json`` via ``save_strategy_defaults``.
...
Usage (from repo root):
    python -m scripts.grid_search_strategy --symbol JPM
    python -m scripts.grid_search_strategy --symbol XOM --top 30 --no-save
"""
...
def run_grid_search(
    ...
    if not os.path.exists(predictions_csv):
        raise FileNotFoundError(
            ...
            f"Run a full backtest first:\n"
            f"  python -m src.backtest --symbol {symbol} --frequency {FREQUENCY} "
            f"--prediction-mode model --report"
        )

    k_sigma_long_vals  = list(_frange(0.20, 0.70, 0.10))
    k_sigma_short_vals = list(_frange(0.30, 0.80, 0.10))
    k_atr_long_vals    = list(_frange(0.25, 0.75, 0.10))
    k_atr_short_vals   = list(_frange(0.30, 0.80, 0.10))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Strategy grid search for any symbol (60min).")
    parser.add_argument("--symbol",        type=str,  required=True, help="Symbol (e.g. JPM, XOM)")
    ...
    df = run_grid_search(symbol=symbol, start_date=args.start_date, end_date=args.end_date)
    ...
    if not args.no_save:
        ...
        save_strategy_defaults(
            ...
            enable_longs=True,
            allow_shorts=True,
            source=f"grid_search_strategy.py --symbol {symbol}",
        )
        print(f"\nSaved to configs/symbols/{symbol}/active.json")
    else:
